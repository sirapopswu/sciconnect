...
// GET all users (including invisible, for admin)
...
app.get("/api/users/search", async (req, res) => {
  ...
  try {
    let query =
      "SELECT id, username, email, major, gender, age, LEFT(student_id, 2) AS generation, photo, bio, skills, line_url, facebook_url, instagram_url FROM users WHERE visible=true";
    let params = [];
...
app.post("/api/users", async (req, res) => {
  ...
  try {
    ...
    if (err.code === "23505")
      return res
        .status(400)
        .json({ success: false, message: "Username หรือ Email ถูกใช้แล้ว" });
...
app.post("/api/users/login", async (req, res) => {
  ...
  try {
    ...
    if (userExist.rows.length === 0)
      return res
        .status(401)
        .json({ success: false, message: "ไม่พบอีเมลนี้ในระบบ" });

    const user = userExist.rows[0];
    if (user.password !== password)
      return res
        .status(401)
        .json({ success: false, message: "รหัสไม่ถูกต้อง" });

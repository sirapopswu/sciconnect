...
# ข้อมูลเดิมจาก phase 1 ,phase 2 ,phase 3
...
# เขียนสรุปการประชุม Retrospective และ Link to Retrospective Youtube video  
ในสปรินต์นี้ ทีมสามารถพัฒนาระบบได้อย่างต่อเนื่อง โดยฝั่ง Frontend สามารถพัฒนาระบบ Admin ได้ประมาณ 80% ขณะที่ฝั่ง Backend สามารถพัฒนา feature หลัก เช่น Login, Signup และ API ต่างๆพร้อมเชื่อมต่อฐานข้อมูลได้สำเร็จ รวมถึงเริ่มมีการเขียน Unit Test เพื่อรองรับการทดสอบระบบ  

สิ่งที่ทำได้ดี :  
Frontend พัฒนาระบบ Admin ได้ถึงประมาณ 80% แล้ว  
แก้ปัญหาเฉพาะหน้าได้ดี โดยเฉพาะด้าน Database และ API  
การสื่อสารในทีมดีขึ้น มีการอัปเดตงานต่อเนื่อง  
เริ่มมีการเขียน Unit Test  

ปัญหาที่พบใน sprint นี้ :  
ติดตั้งเครื่องมือและเชื่อมต่อ localhost/port มีปัญหา  
ทักษะ JavaScript, Node.js, JSON และ Git ยังไม่คล่อง  
โครงสร้าง database / API ไม่ตรงกันช่วงแรก  
Unit Test ยังไม่ชำนาญ และเจอ error จาก handler function (login)  

สิ่งที่ต้องปรับปรุง :  
ควรจะคุยกันให้เคลียก่อนเริ่มทำงาน และควรประสานงานให้มากกว่านี่  
อาจจะต้องเรียนรู้กันเพิ่มเติมอีกเยอะในหลายๆ ส่วนเช่น การเขียน UnitTest, การเขียน test case code และเรียนรู้ทักษะการใช้ Git การเขียน Node.js และ JavaScript เพิ่ม  

สิ่งที่เราได้เรียนรู้ :  
ได้เรียนรู้การคุยกันทำงานเป็น team มากขึ้นทั้งการ scrum และการสื่อสารกันผ่าน github  
เพิ่มทักษะการเขียน JavaScript Node.js และเข้าใจการใช้ Git มากขึ้น  
ได้เข้าใจในการเขียน UnitTest และ TestCaseCode มากขึ้นแต่ยังต้องเรียนรู้เพิ่มอีก  

VIDEO LINK RETROSPECTIVE (WEEK 3) : https://youtu.be/vYX8VEisZUs
...
## 1. Process
- **Continuous Integration (CI):** นำกระบวนการ CI มาใช้ในการตรวจสอบความถูกต้องของโค้ดอัตโนมัติ (Automated Testing) เมื่อมีการอัปเดตโค้ดบน GitHub เพื่อป้องกันไม่ให้ข้อผิดพลาดใหม่กระทบกับระบบที่ทำงานได้อยู่แล้ว
- **Quality Assurance & UI Testing:** เน้นกระบวนการทดสอบส่วนหน้าบ้านและส่วนติดต่อผู้ใช้งาน (UI) โดยจำลองพฤติกรรมการใช้งานจริง (เช่น การคลิก การเข้าสู่ระบบ และการค้นหา) เพื่อยืนยันว่าฟีเจอร์ทุกอย่างทำงานได้อย่างสมบูรณ์แบบ
- **Final Polish & Bug Fixing:** ตรวจสอบและแก้ไข Bug ที่ตกค้างมาจาก Phase ก่อนหน้า (เช่น ปัญหาช่องค้นหา การแสดงผลข้อมูลในโปรไฟล์ และความสมบูรณ์ของ CSS/Layout) พร้อมกับจัดระเบียบโค้ดในโปรเจกต์ (Refactoring) ก่อนส่งมอบระบบ

## 2. Methods
- **Automated Workflows:** กำหนดและตั้งค่า Pipeline การทำงานแบบอัตโนมัติเพื่อทำการรันชุดทดสอบทุกครั้งลดเวลาที่ต้องใช้คนนั่งทดสอบเองให้เหลือน้อยที่สุด
- **End-to-End (E2E) Testing Approach:** ออกแบบ UI Testcases โดยมุ่งเน้นทดสอบระบบแบบครบวงจรตั้งแต่เปิดหน้าแรกไปจนถึงหน้า Profile และหน้าสรุปผล เพื่อดูว่าทั้ง Frontend และ Backend สามารถเชื่อมโยงข้อมูลกันได้ไม่มีสะดุด
- **Final Sprint Tracking:** ติดตามความคืบหน้าของงานช่วงสุดท้ายสไตล์ Agile นำ Task และ Bug ที่เหลืออยู่เข้าไว้บนบอร์ด เพื่อทยอยแก้ไขและปิดงานให้รวดเร็วที่สุด

## 3. Tools
- **UI Testing Frameworks:** เครื่องมือสำหรับการทดสอบ UI อัตโนมัติ จำลองพฤติกรรมเบราว์เซอร์ (เช่น Playwright, Cypress หรือ Selenium ตัวใดตัวหนึ่ง หรืออาศัยชุด UI Test scripts ที่สร้างขึ้นมาตามความเหมาะสม)
- **CI/CD Tools:** GitHub Actions สำหรับรัน Workflow อัตโนมัติ ควบคุมและรายงานสถานะการ Build/Test โดยตรงใน Repository
- **Project Tracking:** GitHub Issues และ GitHub Projects (Kanban) สำหรับจัดการและติดตามบั๊กในโค้งสุดท้าย
- **Coding & Debugging:** VS Code, Browser Developer Tools และ Postman ใช้งานควบคู่กันในการดีบัก API และปรับแต่ง CSS ของหน้าเว็บไซต์

...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    if (user) {
      ...
      if (window.location.pathname.includes("profile.html")) {
        ...
        if (
          ...
          String(targetId) !== String(user.student_id)
        ) {
          try {
        ...
        const derivedGen =
          user.generation ||
          (user.student_id ? user.student_id.substring(0, 2) : "-");
        ...
        const dSocialLinksContainer = document.getElementById(
        ...
        if (profileCard && user.major) {
          ...
          if (dAvatar)
            dAvatar.className = `profile-avatar themed border-${user.major}`;
          if (dMajor)
            dMajor.className = `profile-branch-name themed text-${user.major}`;
        }
        ...
        if (
          ...
          if (
            String(loggedInUser.id) === String(user.id) ||
            String(loggedInUser.student_id) === String(user.student_id) ||
      ...
      visibilityToggle.addEventListener("change", async (e) => {
        ...
        try {
          const visRes = await fetch(
            `http://localhost:3000/api/users/${user.id}/visibility`,
            {
              method: "PATCH",
    ...
    if (editFormPage && user) {
      ...
      editFormPage.addEventListener("submit", async (e) => {
        ...
        const facebook_url = document.getElementById("facebook_url")
          ? document.getElementById("facebook_url").value.trim()
          : "";
        const instagram_url = document.getElementById("instagram_url")
          ? document.getElementById("instagram_url").value.trim()
          : "";
        const line_url = document.getElementById("line_url")
          ? document.getElementById("line_url").value.trim()
          : "";
        ...
        try {
          ...
          if (res.ok) {
            ...
            showToast(
              "เกิดข้อผิดพลาด: " + (data.message || "Unknown error"),
              "error",
            );
            const saveBtn = editFormPage.querySelector(".btn-save");
            if (saveBtn) {
              saveBtn.textContent = "บันทึกข้อมูลโพรไฟล์ (Save)";
  ...
  if (membersGrid && isMajorPage) {
    ...
    if (major) {
      membersGrid.innerHTML =
        '<div style="grid-column: 1/-1; text-align: center; padding: 2rem; color: #64748b; font-size: 1.2rem;"><svg class="sci-svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="3"></circle><ellipse cx="12" cy="12" rx="10" ry="4" transform="rotate(45 12 12)"></ellipse><ellipse cx="12" cy="12" rx="10" ry="4" transform="rotate(-45 12 12)"></ellipse></svg> กำลังดึงข้อมูลเพื่อนๆ สาขา ' +
        major.toUpperCase() +
        "...</div>";
  ...
  const majorLink = Array.from(document.querySelectorAll(".nav-links a")).find(
    (a) => a.getAttribute("href") === "major.html",
  );
  if (majorLink) {
    const path = window.location.pathname.toLowerCase();
    const isConnectPage =
      path.includes("major.html") ||
...
function injectProfileModal() {
  ...
  document
    .getElementById("profileModalBackdrop")
  ...
  content.innerHTML = `
        ...
        <div class="modal-skills">
            ${
              (skills || []).length > 0
                ? skills
                    .map((s) => `<span class="modal-skill-tag">${s}</span>`)
                    .join("")
...
async function loadMajorStudents(major, container) {
  try {
    const res = await fetch(
      `http://localhost:3000/api/users/search?major=${major}`,
    );
    ...
    if (users.length === 0) {
      container.innerHTML =
        '<div style="grid-column: 1/-1; text-align: center; padding: 3rem; color: #64748b;">ยังไม่มีนิสิตในสาขานี้ที่เปิดเผยโปรไฟล์</div>';
    } else {
...
function createStudentCard(user) {
  ...
  const avatar =
    user.photo && user.photo !== "default.png"
      ? user.photo
      : `https://ui-avatars.com/api/?name=${avatarName}&background=random&color=fff&size=128`;
  ...
  card.innerHTML = `
        ...
        <div style="display: flex; flex-wrap: wrap; gap: 4px; justify-content: center; margin-top: 0.5rem;">
            ${(skills || [])
              ...
              .map(
                (s) =>
                  `<span style="font-size: 0.7rem; background: white; padding: 2px 6px; border-radius: 4px; border: 1px solid #e2e8f0;">${s}</span>`,
              )
              .join("")}
        </div>
    `;
...
function injectSearchUI() {
  ...
  // Close on backdrop click
  document
    .getElementById("searchModalBackdrop")
    .addEventListener("click", (e) => {
...
async function performSearch(fromUrl = false) {
  ...
  if (searchResults) {
    ...
    try {
      ...
      if (users.length === 0) {
        searchResults.innerHTML =
          '<div style="grid-column: 1/-1; text-align: center; padding: 3rem; color: #64748b;">ไม่พบข้อมูลนิสิตที่ค้นหา</div>';
      } else {
      ...
      if (branchesSection)
        branchesSection.style.display = isSearching ? "none" : "block";
      ...
      searchResults.innerHTML =
        '<div style="grid-column: 1/-1; text-align: center; padding: 3rem; color: #ef4444;">เกิดข้อผิดพลาดในการโหลดข้อมูล</div>';
    }
  }
}
